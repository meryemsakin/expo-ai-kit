...
export default function AndroidSetupPage() {
  return <DocsLayout>
    <h1>Android setup</h1>
    <p>Follow <Link href="/get-started">Get Started</Link> to install and build. Android requires API 26 or newer. Text generation needs no plugin options; availability depends on the device and model.</p>
    <h2 id="configuration">Optional features</h2>
  ...
  "expo": {
    ...
    <CodeBlock language="properties">{`# Enable image operations and face checks:
expoAiKit.vision=true`}</CodeBlock>
    <p>The other properties are <code>expoAiKit.speech</code> and <code>expoAiKit.androidEmbeddings</code>. Speech also needs <code>android.permission.RECORD_AUDIO</code> in your manifest. Set your app&apos;s <code>minSdkVersion</code> to at least 26.</p>
